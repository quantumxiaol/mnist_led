...
# ----------------------------
batch_size = 64
learning_rate = 0.1  # 与C++保持一致
num_epochs = 40      # 与C++保持一致
...
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Lambda(lambda x: x.view(-1))  # 展平为784维向量，与C++保持一致
])
...
for epoch in range(num_epochs):
    ...
    # 每轮都打印，与C++保持一致
    print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {avg_loss:.6f}, Accuracy: {accuracy:.1f}%')
...
model_path = os.path.join(weights_dir, 'simple_nn_mnist.pth')
